...
{
	#if defined(STACK_USE_HTTP_SERVER) || defined(STACK_USE_HTTP2_SERVER)
#error "STACK_USE_HTTP_SERVER"
        HTTPServer();
	#endif
	
	#if defined(STACK_USE_FTP_SERVER) && defined(STACK_USE_MPFS)
#error "STACK_USE_FTP_SERVER"
        FTPServer();
	#endif
	
	#if defined(STACK_USE_SNMP_SERVER)
#error "STACK_USE_SNMP_SERVER"
        SNMPTask();
	#endif
	
	#if defined(STACK_USE_ANNOUNCE)
#error "STACK_USE_ANNOUNCE"
        DiscoveryTask();
	#endif
	
	#if defined(STACK_USE_NBNS)
//#error "STACK_USE_NBNS"
        NBNSTask();
	#endif
	
	#if defined(STACK_USE_DHCP_SERVER)
#error "STACK_USE_DHCP_SERVER"
        DHCPServerTask();
	#endif
	
	#if defined(STACK_USE_DNS_SERVER)
#error "STACK_USE_DNS_SERVER"
        DNSServerTask();
	#endif
	
	#if defined (STACK_USE_DYNAMICDNS_CLIENT)
#error "STACK_USE_DYNAMICDNS_CLIENT"
        DDNSTask();
	#endif
	
	#if defined(STACK_USE_TELNET_SERVER)
#error "STACK_USE_TELNET_SERVER"
        TelnetTask();
	#endif
	
	#if defined(STACK_USE_REBOOT_SERVER)
//#error "STACK_USE_REBOOT_SERVER"
        RebootTask();
        #endif
	
	#if defined(STACK_USE_SNTP_CLIENT)
//#error "STACK_USE_SNTP_CLIENT"
        SNTPClient();
        #endif
	
	#if defined(STACK_USE_UDP_PERFORMANCE_TEST)
#error "STACK_USE_UDP_PERFORMANCE_TEST"
        UDPPerformanceTask();
	#endif
	
	#if defined(STACK_USE_TCP_PERFORMANCE_TEST)
#error "STACK_USE_TCP_PERFORMANCE_TEST"
        TCPPerformanceTask();
	#endif
	
	#if defined(STACK_USE_SMTP_CLIENT)
#error "STACK_USE_SMTP_CLIENT"
        SMTPTask();
	#endif
	
	#if defined(STACK_USE_UART2TCP_BRIDGE)
#error "STACK_USE_UART2TCP_BRIDGE"
        UART2TCPBridgeTask();
	#endif
}
